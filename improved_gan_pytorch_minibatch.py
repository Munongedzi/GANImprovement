# -*- coding: utf-8 -*-
"""improved_GAN_pytorch_miniBatch.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DcFPEUPuY1vsCqB6gKj-Uz653PI0wJZr
"""
...
epochs = 50 # number of epochs (training)
...
outf = './fake_minibatch' #folder to output images and model checkpoints
log_interval = 1000
test_interval = 5000
...
if os.path.isdir('./fake_minibatch'):
   pass
else:
   os.mkdir('./fake_minibatch')
...
class MiniBatchDiscrimination(nn.Module):
    def __init__(self, in_features, out_features, num_kernels, kernel_dim):
        ...
        self.T = nn.Parameter(torch.Tensor(in_features, num_kernels, kernel_dim))
        nn.init.normal_(self.T, 0, 1)  # Initialize with normal distribution

    def forward(self, x):
        M = x.matmul(self.T)
        M = M.unsqueeze(0) - M.unsqueeze(1)
        M = torch.exp(-torch.sum(torch.abs(M), dim=2))
        return torch.cat([x, M.mean(1)], 1)
...
class _netD(nn.Module):
    def __init__(self):
        ...
        self.mb_disc = MiniBatchDiscrimination(512, 512, num_kernels=100, kernel_dim=5)
...
def test():
    ...
    # Assuming fsave is already an open file object
...
for epoch in range(1, epochs + 1):
    for i, data in enumerate(dataloader):
        ...
        print('[%d/%d][%d/%d] Loss_label: %.4f Loss_unlabel_real: %.4f Loss_fake: %.4f Loss_D: %.4f Loss_G: %.4f' % (epoch, epochs, i, len(dataloader),
        loss_label.item(), loss_unl_real.item(), loss_unl_fake.item(), loss_D.item(), loss_G.item()))
...
fixed_noise = torch.FloatTensor(batch_size, nz, 1, 1).normal_(0, 1)  # Fixed noise
fake_images = netG(fixed_noise)  # Generate fake images using the trained generator

# Save or display the generated images
import torchvision.utils as vutils
vutils.save_image(fake_images.data, 'generated_images.png', normalize=True)
...
Original file is located at
    https://colab.research.google.com/drive/1csbYFgVv5hiWFgFa7n7NR5OmgFC5BBKW
"""
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(500, 500),
            nn.ReLU(),
            nn.Linear(500, 3 * 32 * 32),  # Output size: 3x32x32 image
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(500, 250),
            nn.ReLU(),
            nn.Linear(250, 10)  # 10 classes for SVHN dataset
        )
...
def save_generated_images(generator, folder_path='generated_images', num_images=5):
    #os.makedirs(folder_path, exist_ok=True)  # Create folder if it doesn't exist
    # Generate random noise
    noise = torch.randn(num_images, 100)
    # Pass noise through the generator
    generated_images = generator(noise).view(-1, 3, 32, 32).cpu().detach()

    # Normalize and plot images
    generated_images = (generated_images * 0.5) + 0.5  # Undo normalization for display
...
for epoch in range(300):
    ...
    print(f"Epoch {epoch+1}, Time: {time.time() - start_time:.2f}s, Labeled Loss: {loss_lab:.4f}, Unlabeled Loss: {loss_unl:.4f}, Train Error: {train_err:.4f}, Test Error: {test_err:.4f}")
        ...
        #save_generated_images(gen, folder_path='generated_images')
